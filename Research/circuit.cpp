...
bool ChooseVertexWithGreedyMDS(int year,double pre_rvalue){
	...
	while (chk){
		...
		int ii = 0;
		while (ii < cand.size() && rand()%10 == 9){	//10%的機會跳到較差的解
			ii++;
		}
		int temp = cand[ii].pn;
		PathC[temp]->SetChoose(true);
		if (hashp.Exist()){
			PathC[temp]->SetChoose(false);
			for (ii = 0; ii < cand.size(); ii++){
				PathC[cand[ii].pn]->SetChoose(true);
				if (!hashp.Exist())
					break;
				PathC[cand[ii].pn]->SetChoose(false);
			}
			if (ii >= cand.size())
				return false;
			PathC[cand[ii].pn]->SetChoose(false);
